...
- **数据处理层 (Python)**：
  ...
  - `src/process_poets.py`：统计高产诗人在悲喜、山水、边塞等 6 个古典维度的词频密度（按各诗人总词数归一化，消除作品篇数偏差），生成 `web/data/poet_sentiment.json`。
  - `src/process_clusters.py`：提取 52 位代表诗人的 150 维 TF-IDF 词袋特征，引入 TruncatedSVD (8D LSA) 平滑特征空间，执行 KMeans (K=4) 聚类与 t-SNE 二维流形降维，输出兼顾全局结构与局部岛屿分布的 `web/data/poet_clusters.json`。
...
- **交互可视化层 (D3.js v5 + Vite)**：
  ...
  - **Tab 3 - 风格流形 (clusters.html)**：t-SNE 二维散点群与平滑凸包水彩疆域（Watercolor Polygon Hulls），配备八方向自适应投射避让算法与平移缩放（Zoom & Pan）。
...
│   └── process_clusters.py         # TF-IDF + SVD + KMeans + t-SNE 聚类流形
├── tests/
│   ├── test_corpus_parser.py       # 语料分卷状态机与元数据泄露防护测试
│   ├── test_clustering_quality.py  # 聚类流派常识与轮廓系数阈值测试
│   └── test_data_contracts.py      # 前后端数据契约对齐与完整性测试
└── web/
    ├── index.html                  # Tab 1: 意象词频柱状图
    ├── poets.html                  # Tab 2: 诗人六维画像极坐标图
    ├── clusters.html               # Tab 3: 诗人风格聚类流形散点图
    ├── css/
    │   └── style.css               # 古风统一样式、Halo 描边与移动端响应式规则
    ├── js/
    │   ├── app.js                  # Tab 1 逻辑
    │   ├── poets.js                # Tab 2 逻辑
